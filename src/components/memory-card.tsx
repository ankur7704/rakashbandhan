...
const MemoryCard = ({ memory, isActive, onClick }: MemoryCardProps) => {
  return (
    ...
    >
      <Card
        className={cn("overflow-hidden bg-white p-2 pb-4 shadow-lg transition-all duration-300 ease-in-out h-full flex flex-col", isActive ? "shadow-2xl scale-100" : "shadow-lg scale-95 opacity-80" )}
      >
        ...
        <CardContent className="p-0 flex flex-col flex-grow">
          ...
          <blockquote className="mt-4 px-2 text-center font-headline text-base italic text-gray-700 border-l-4 border-primary/50 pl-4 flex-grow">
            <p className="line-clamp-4">{memory.wish}</p>
          </blockquote>
        </CardContent>
      </Card>
    </div>
